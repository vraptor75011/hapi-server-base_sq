...
const filters = {
	id: Joi.alternatives().try(
		Joi.array().description('the role ID PK increment: 1, [1,2]')
      .items(Joi.number().integer().min(1)),
		Joi.number().integer().min(1)).description('the role ID PK increment: 1, [1,2]'),
	name: Joi.string().min(3).max(64).description('the role name'),
