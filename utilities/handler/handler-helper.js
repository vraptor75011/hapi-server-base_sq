...
module.exports = {
     ...
     * @param model: A mongoose model name.
                ...
                if (query.$min || query.$max || query.$sum) {
                    ...
                    return model.count(sequelizeQuery)
                        .then(function (result) {
                            ...
                            if (query.$min) {
                                return model.min(attr, sequelizeQuery)
                                    .then(function (result) {
                                        Log.apiLogger.info("Result: %s", JSON.stringify(result));
                                        const items = {
                                            total: totalCount,
                                    ...
                                    .catch(function (error) {
                                        const message = "Error in filtered min operation";
                                        ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST);
                                    })
                            }
                            if (query.$max) {
                                return model.max(attr, sequelizeQuery)
                                    .then(function (result) {
                                        Log.apiLogger.info("Result: %s", JSON.stringify(result));
                                        const items = {
                                            total: totalCount,
                                    ...
                                    .catch(function (error) {
                                        const message = "Error in filtered max operation";
                                        ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST);
                                    })
                            }
                            if (query.$sum) {
                                return model.sum(attr, sequelizeQuery)
                                    .then(function (result) {
                                        Log.apiLogger.info("Result: %s", JSON.stringify(result));
                                        const items = {
                                            total: totalCount,
                                    ...
                                    .catch(function (error) {
                                        const message = "Error in filtered sum operation";
                                        ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST);
                                    })
                            }
                        })
                        .catch(function (error) {
                            const message = "Error in filtered math operation";
                            ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST);
                        })
                }

                // QUERY for all the rest
                // Lower priority, with filter, pagination, sort and relations.
                // Only the right URL query params will use to building the Seq. query string
                queryInclude = queryFilteredRest(query, model);
                sequelizeQuery = QueryHelper.createSequelizeFilter(model, queryInclude, sequelizeQuery);
                return model.count(sequelizeQuery)
                    .then(function (result) {
                        filteredCount = result;
                        ...
                        sequelizeQuery = QueryHelper.createSequelizeFilter(model, querySort, sequelizeQuery);
                        ...
                        return model.findAll(sequelizeQuery)
                            ...
                            .catch(function (error) {
                                const message = "Error in findAll operation";
                                ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST);
                            })
                    })
                    .catch(function (error) {
                        const message = "Error in filtered findAll operation";
                        ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST);
                    })
            })
            .catch(function (error) {
                const message = "Error in first count.";
                ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST);
            });


        sequelizeQuery = model.find();
        sequelizeQuery = QueryHelper.createMongooseQuery(model, query, sequelizeQuery).lean();
        return sequelizeQuery.count()
            .then(function (result) {
                count = result;
                sequelizeQuery = QueryHelper.paginate(query, sequelizeQuery);
                return sequelizeQuery.exec('find')
            })
            .then(function (result) {
