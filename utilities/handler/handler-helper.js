...
async function _create(model, payload) {
    try {
        ...
        const message = 'There was an error processing the CREATE '+ model.name+' request.';
        ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST)
    }
}
...
async function _update(model, id, payload) {
    try {
        ...
        const message = 'There was an error processing the UPDATE ' + model.name + ' request.';
        ErrorHelper.handleError(error, message, ErrorHelper.types.BAD_REQUEST)
    }
}
 ...
 * @private
 */
function _deleteOne(model, id, hardDelete) {
    try {
        let sequelizeQuery = {where: {id: id}};

        return model.destroy(sequelizeQuery)
            .then(function () {
                return true
            })
            .catch(function (error) {
                const message = "There was a postprocessing error during deleting the resource.";
