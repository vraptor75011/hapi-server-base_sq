...
import GeoRepartitionForm from './GeoRepartitionForm';
...
class GeoRepartitionsTable extends React.PureComponent {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            columns: [
                {
                    Header: props.t('app:geoRepartition.name'),
                    accessor: 'name',
                    minWidth: 200,
    ...
    fetchData = debounce(state => {
        ...
        this.props.getGeoRepartitions(params);
    }, 300);
    ...
    render() {
        const { classes, geoRepartitions, modal, t } = this.props;
        ...
        return (
            <div>
                {modal && (
                    <GeoRepartitionForm {...this.state} {...this.props} cancel={this.cancel} />
                )}
                <Paper className={classes.root}>
                    <ReactTable
                        columns={columns}
                        manual // Forces table not to paginate or sort automatically, so we can handle it server-side
                        data={geoRepartitions.docs ? geoRepartitions.docs : []}
                        pages={geoRepartitions.pages ? geoRepartitions.pages.total : 1} // Display the total number of pages
                        ...
                        defaultPageSize={geoRepartitions.items ? geoRepartitions.items.total : 3}
...
export default (GeoRepartitionsTable);
